            ...
            <div class="bg-slate-900/80 rounded-2xl border border-white/10 shadow-xl shadow-indigo-500/10 p-6 sm:p-8">
                <div class="flex items-center justify-between gap-4 flex-wrap mb-6">
                    <div>
                        ...
                        <p class="text-sm text-indigo-100/80 mt-2">Pilih barang habis pakai yang tersedia, stok akan langsung berkurang.</p>
                    </div>
                ...
                <form method="POST" action="{{ route('peminjam.barang-habis-pakai.store') }}" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="md:col-span-2">
                            ...
                            <select name="idbarang" class="w-full px-3 py-2 rounded-xl bg-slate-800/70 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent">
                    ...
                    <div class="flex items-center justify-end gap-3">
                        <button type="submit" class="inline-flex items-center px-5 py-2.5 rounded-xl bg-white text-indigo-700 font-semibold shadow hover:shadow-indigo-500/40 transition">
                            Kirim Request
                        </button>
                    </div>
                </form>
            ...
            <div class="bg-slate-900/80 rounded-2xl border border-white/10 shadow-xl shadow-indigo-500/10 p-6 sm:p-8">
                <div class="flex items-center justify-between gap-4 flex-wrap mb-4">
                    <div>
                        ...
                        <p class="text-sm text-indigo-100/70">Daftar barang habis pakai yang pernah Anda ambil.</p>
                    </div>
                </div>
                        ...
                        <tbody class="divide-y divide-white/5">
                            @forelse($riwayat as $row)
                                <tr class="hover:bg-white/5">
                                    ...
                                    <td class="px-4 py-3 text-sm text-slate-300">{{ $row->keterangan ?? '-' }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="px-4 py-6 text-center text-sm text-slate-300">Belum ada request barang habis pakai.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>

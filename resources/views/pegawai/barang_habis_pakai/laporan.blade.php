                ...
                <tbody>
                    ...
                    @forelse($riwayat as $row)
                        @php
                            ...
                            $tanggalKeluar = $row->tgl_keluar ?? '-';
                            $tanggalDiterima = $row->tgl_diterima
                                ? \Illuminate\Support\Carbon::parse($row->tgl_diterima)->format('d-m-Y H:i')
                                : '-';
                        @endphp
                        <tr>
                            ...
                            <td class="category-cell">{{ $row->user?->divisi ?? '-' }}</td>
                            <td class="stock-cell">{{ $row->jumlah ?? 0 }}</td>
                            <td class="category-cell">
                                <span class="badge {{ $badgeClass }}">{{ strtoupper($status) }}</span>
                            </td>
                            <td class="category-cell">{{ $row->tgl_pengambilan_rencana ?? '-' }}</td>
                            <td class="category-cell">{{ $tanggalDiterima }}</td>
                            <td>{{ $row->keterangan ?? '-' }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="10" class="muted" style="text-align: center; padding: 10px;">
    ...
    @php
        ...
        $petugasInventaris =
            $usePdfConfig && $appConfig && $appConfig->petugas_inventaris
                ...
                : 'Nama Petugas';
        $totalKeluar = $riwayat->sum('jumlah');
...
">
    ...
    ">
        ...
        width: 240px;
    ">
            <span style="font-size: 10pt; color: #555;">Total Barang Keluar</span>
            <span style="font-size: 10pt; font-weight: 700; color: #1e3a8a;">{{ $totalKeluar }} unit</span>
        </div>
    </div>

    ...
    <div class="min-h-screen bg-slate-950 text-white">
            ...
            <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-16">
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-5">
                    <div>
                        ...
                        <p class="mt-3 text-indigo-50/90 max-w-2xl">Pantau penyebaran aset tetap per ruang, unduh laporan,
                            dan jaga keterisian ruang tetap terukur.</p>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-8">
                    <div
                        class="relative overflow-hidden rounded-2xl border border-white/10 bg-white/10 backdrop-blur p-5 shadow-lg shadow-indigo-500/20">
        ...
        <div class="relative -mt-10 pb-16">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
                <div
                    class="bg-slate-900/70 border border-white/10 rounded-2xl shadow-lg shadow-indigo-500/10 backdrop-blur">
                    <form method="GET" class="p-6 space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-4 gap-4">
                            <div>
                                ...
                                <select name="gedung"
                                    ...
                                    @foreach ($gedungList ?? [] as $g)
                                        <option class="text-slate-900" value="{{ $g }}"
                                            @selected(request('gedung') == $g)>{{ $g }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="lantai"
                                    ...
                                    @foreach ($lantaiList ?? [] as $l)
                                        <option class="text-slate-900" value="{{ $l }}"
                                            @selected(request('lantai') == $l)>{{ $l }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                <label class="text-sm font-semibold text-indigo-100 mb-2 block">Filter Ruang</label>
                                <select name="idruang" id="ruangSelect"
                                    class="w-full rounded-xl bg-slate-800/60 border border-white/10 text-white focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400">
                                    ...
                                    @foreach ($ruangOptions as $r)
                                        <option class="text-slate-900" data-gedung="{{ $r->nama_gedung }}"
                                            data-lantai="{{ $r->nama_lantai }}" value="{{ $r->idruang }}"
                            ...
                            <div>
                                ...
                                <select name="idbarang"
                                    ...
                                    @foreach ($barang as $b)
                                        <option class="text-slate-900" value="{{ $b->idbarang }}"
                                            @selected(request('idbarang') == $b->idbarang)>{{ $b->nama_barang }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="status"
                                    ...
                                    <option class="text-slate-900" value="kurang_baik" @selected(request('status') === 'kurang_baik')>Kurang
                                        Baik</option>
                                    <option class="text-slate-900" value="rusak" @selected(request('status') === 'rusak')>Rusak
                                    </option>
                                </select>
                            </div>
                        </div>
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            <div
                                class="flex items-center gap-2 bg-slate-800/60 border border-white/10 rounded-xl px-3 py-2">
                                <label class="text-sm font-semibold text-indigo-100">Tampil</label>
                                <select name="per_page"
                                    class="select-perpage rounded-lg bg-slate-900/60 border border-white/10 text-white px-2 py-1 pr-7 focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400">
                                    @foreach ([10, 15, 25, 50, 100] as $pp)
                                        <option class="text-slate-900" value="{{ $pp }}"
                                            @selected(request('per_page', 15) == $pp)>{{ $pp }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="flex flex-wrap items-center gap-3">
                                <button type="submit"
                                    class="inline-flex items-center justify-center px-4 py-2.5 rounded-xl bg-gradient-to-r from-amber-400 to-orange-500 text-slate-900 font-semibold shadow-lg shadow-amber-500/30 hover:shadow-amber-500/50 transition">
                                    Terapkan
                                </button>
                                <button type="submit"
                ...
                @if (request('idruang') || request('idbarang') || request('gedung') || request('lantai') || request('status'))
                    <div class="px-6 pb-4 flex flex-wrap gap-2 text-sm text-indigo-100">
                        ...
                        @if (request('gedung'))
                            <span class="px-3 py-1 rounded-full bg-purple-500/30 border border-purple-200/30">Gedung:
                                {{ request('gedung') }}</span>
                        @endif
                        @if (request('lantai'))
                            <span class="px-3 py-1 rounded-full bg-fuchsia-500/30 border border-fuchsia-200/30">Lantai:
                                {{ request('lantai') }}</span>
                        @endif
                        @if (request('idruang'))
                            <span class="px-3 py-1 rounded-full bg-indigo-500/30 border border-indigo-200/30">Ruang:
                                {{ $ruang->firstWhere('idruang', request('idruang'))?->nama_ruang ?? 'Dipilih' }}</span>
                        @endif
                        @if (request('idbarang'))
                            <span class="px-3 py-1 rounded-full bg-sky-500/30 border border-sky-200/30">Barang:
                                {{ $barang->firstWhere('idbarang', request('idbarang'))?->nama_barang ?? 'Dipilih' }}</span>
                        @endif
                        @if (request('status'))
                            @php
                                $labelStatus =
                            ...
                            @endphp
                            <span
                                class="px-3 py-1 rounded-full bg-rose-500/30 border border-rose-200/30">{{ $labelStatus }}</span>
                        @endif
                        <a href="{{ route(($routePrefix ?? 'pegawai') . '.inventaris-ruang.index') }}"
                            class="px-3 py-1 rounded-full bg-white/10 border border-white/10 hover:bg-white/20 transition">Reset</a>
                    </div>
                @endif

                <div
                    class="bg-slate-900/80 border border-white/10 rounded-2xl shadow-xl shadow-indigo-500/15 overflow-hidden">
                    <div class="grid gap-6 p-6 md:grid-cols-2 xl:grid-cols-3">
                        @forelse($units as $unit)
                            @php
                                ...
                                $isKurangBaik =
                                    !$isRusak &&
                                    $unit->keterangan &&
                                    str_contains(strtolower($unit->keterangan), 'kurang');
                            ...
                            @endphp
                            <div
                                class="group relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900/80 via-slate-900/60 to-slate-800/70 backdrop-blur shadow-lg shadow-indigo-500/15">
                                <div
                                    class="absolute inset-0 opacity-60 bg-[radial-gradient(circle_at_20%_20%,rgba(99,102,241,0.25),transparent_35%),radial-gradient(circle_at_80%_0%,rgba(14,165,233,0.25),transparent_35%)]">
                                </div>
                                <div class="relative p-5 space-y-4">
                                    <div class="flex items-start justify-between gap-3">
                                        <div>
                                            <p class="text-xs uppercase tracking-[0.3em] text-indigo-100/70">Unit
                                                {{ str_pad($unit->nomor_unit, 3, '0', STR_PAD_LEFT) }}</p>
                                            <h3 class="text-xl font-bold text-white mt-1 leading-tight">
                                                {{ $barang->nama_barang ?? '-' }}</h3>
                                            <p class="text-sm text-indigo-100/80">{{ $ruangItem->nama_ruang ?? '-' }} •
                                                {{ $ruangItem->nama_gedung ?? '' }}{{ $ruangItem->nama_lantai ? ' Lt. ' . $ruangItem->nama_lantai : '' }}
                                            </p>
                                        </div>
                                        <span
                                            class="px-3 py-1 rounded-full bg-white/10 border border-white/10 text-xs font-semibold text-indigo-50">{{ $unit->kode_unit }}</span>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm text-indigo-100/80">
                                        <div class="rounded-xl border border-white/5 bg-white/5 p-3">
                                            <p class="text-[11px] uppercase tracking-[0.2em] text-indigo-200/70">Kategori
                                            </p>
                                        ...
                                        <div class="rounded-xl border border-white/5 bg-white/5 p-3">
                                            ...
                                            <p class="text-sm font-semibold {{ $statusClass }} mt-1">{{ $statusLabel }}
                                            </p>
                                            @if ($isRusak && $unit->kerusakanAktif?->deskripsi)
                                                <p class="text-xs text-rose-100/80 mt-0.5">
                                                    {{ $unit->kerusakanAktif->deskripsi }}</p>
                                            @elseif($isKurangBaik)
                                    ...
                                    @if ($hasSpec)
                                        <div class="rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-4">
                                            <div class="flex items-center justify-between mb-2">
                                                <p class="text-xs font-semibold text-indigo-100 uppercase tracking-wide">
                                                    Detail Spesifikasi</p>
                                                @if ($unitMasuk?->tgl_masuk)
                                                    <span
                                                        class="text-[11px] px-2 py-1 rounded-full bg-white/5 border border-white/10 text-indigo-100/80">Datang
                                                        {{ \Carbon\Carbon::parse($unitMasuk->tgl_masuk)->format('d M Y') }}</span>
                                                @endif
                                            </div>
                                            <dl class="grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-indigo-100/80">
                                                <div>
                                                    <dt class="text-[11px] uppercase tracking-[0.15em] text-indigo-200/70">
                                                        Prosesor</dt>
                                                    <dd class="font-semibold text-white">
                                                        {{ $unitMasuk?->processor ?? '—' }}</dd>
                                                </div>
                                                <div>
                                                    <dt class="text-[11px] uppercase tracking-[0.15em] text-indigo-200/70">
                                                        RAM</dt>
                                                    <dd class="font-semibold text-white">
                                                        @if ($unitMasuk?->ram_capacity_gb)
                                                            {{ $unitMasuk->ram_capacity_gb }} GB
                                                            {{ $unitMasuk->ram_brand ? '(' . $unitMasuk->ram_brand . ')' : '' }}
                                                        @else
                                                            —
                                                        @endif
                                                    </dd>
                                                ...
                                                <div>
                                                    ...
                                                    <dd class="font-semibold text-white">
                                                        @if ($unitMasuk?->storage_capacity_gb)
                                                            {{ $unitMasuk->storage_capacity_gb }} GB
                                                            {{ $unitMasuk->storage_type ?? '' }}
                                                        @else
                                                            —
                                                        @endif
                                                    </dd>
                                                ...
                                                <div>
                                                    ...
                                                    <dd class="font-semibold text-white">
                                                        @if ($unitMasuk?->monitor_brand)
                                                            {{ $unitMasuk->monitor_brand }}
                                                            {{ $unitMasuk->monitor_model }}
                                                            {{ $unitMasuk->monitor_size_inch ? $unitMasuk->monitor_size_inch . '”' : '' }}
                                                        @else
                                                            —
                                                        @endif
                                                    </dd>
                                            ...
                                            @if ($unit->keterangan)
                                                <p class="mt-3 text-sm text-indigo-100/80"><span
                                                        class="font-semibold text-indigo-100">Catatan:</span>
                                                    {{ $unit->keterangan }}</p>
                                            @endif
                                        </div>
                                    @elseif($unit->keterangan)
                                        <div class="rounded-xl border border-white/10 bg-white/5 p-4">
                                            <p class="text-xs font-semibold text-indigo-100 uppercase tracking-wide mb-1">
                                                Catatan</p>
                                    ...
                                    <div class="flex items-start justify-between pt-1 flex-wrap gap-3">
                                        ...
                                        <div class="flex flex-wrap items-center gap-2">
                                            <form
                                                action="{{ route(($routePrefix ?? 'pegawai') . '.inventaris-ruang.move', $unit) }}"
                                                method="POST" class="flex items-center gap-2">
                                                @csrf
                                                <select name="idruang" required
                                                    class="rounded-lg bg-slate-800/60 border border-white/10 text-white text-xs px-2 py-1 focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400">
                                                    <option class="text-slate-900" value="">Pindah ke...</option>
                                                    @foreach ($ruangAll ?? $ruang as $r)
                                                        <option class="text-slate-900" value="{{ $r->idruang }}"
                                                            @selected($r->idruang == $unit->idruang)>
                                                            {{ $r->nama_ruang }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                <button type="submit"
                                                    class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-indigo-500/80 hover:bg-indigo-500 text-white shadow-md shadow-indigo-500/30 transition">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2" d="M4 12h16m-7-7 7 7-7 7" />
                                                    </svg>
                                                    Pindahkan
                                                </button>
                                            </form>
                                            <a href="{{ route(($routePrefix ?? 'pegawai') . '.inventaris-ruang.label', $unit) }}"
                                                target="_blank"
                                                ...
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M7 7h10M7 11h10M7 15h6M5 7V5a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v2m0 10v2a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2v-2" />
                                                </svg>
                                                Label
                                            </a>
                                            <form
                                                action="{{ route(($routePrefix ?? 'pegawai') . '.inventaris-ruang.destroy', $unit) }}"
                                                method="POST" data-confirm="Hapus unit ini?">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-rose-500/80 hover:bg-rose-500 text-white shadow-md shadow-rose-500/30 transition">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
                                                            d="M19 7l-.867 12.142A2 2 0 0 1 16.138 21H7.862a2 2 0 0 1-1.995-1.858L5 7m5 4v6m4-6v6m1-10V5a2 2 0 0 0-2-2h-2a2 2 0 0 0-2 2v2H5m14 0H5" />
                                                    </svg>
                                                    Hapus
                                                </button>
                                            </form>
                                            @unless ($isRusak)
                                                <form
                                                    action="{{ route(($routePrefix ?? 'pegawai') . '.inventaris-ruang.mark-rusak', $unit) }}"
                                                    method="POST" data-confirm="Tandai unit ini rusak?"
                                                    class="inline-flex items-center gap-2">
                                                    @csrf
                                                    <button type="submit"
                                                        class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-amber-500/80 hover:bg-amber-500 text-slate-900 font-semibold shadow-md shadow-amber-500/30 transition">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                            viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                stroke-width="2"
                                                                d="M12 8v4m0 4h.01M4.93 4.93a10 10 0 1 1 14.14 14.14A10 10 0 0 1 4.93 4.93z" />
                                                        </svg>
                                                        Tandai Rusak
                                                    </button>
                                                </form>
                                            @else
                                                <form
                                                    action="{{ route(($routePrefix ?? 'pegawai') . '.inventaris-ruang.update-kerusakan', $unit) }}"
                                                    method="POST" class="flex flex-wrap items-center gap-2">
                                                    @csrf
                                                    @method('PATCH')
                                                    <input type="text" name="deskripsi"
                                                        value="{{ $unit->kerusakanAktif?->deskripsi }}"
                                                        placeholder="Catatan kerusakan"
                                                        class="px-2 py-1 rounded-lg bg-slate-800/60 border border-white/10 text-white text-xs focus:ring-2 focus:ring-amber-400 focus:border-amber-400 w-44 sm:w-56">
                                                    <button type="submit"
                                                        class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-amber-500/80 hover:bg-amber-500 text-slate-900 font-semibold shadow-md shadow-amber-500/30 transition text-xs">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                            viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                ...
                                                <form
                                                    action="{{ route(($routePrefix ?? 'pegawai') . '.inventaris-ruang.restore-kerusakan', $unit) }}"
                                                    method="POST" data-confirm="Pulihkan unit ini ke kondisi baik?">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                                                        class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-emerald-500/80 hover:bg-emerald-500 text-white font-semibold shadow-md shadow-emerald-500/30 transition">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                            viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                stroke-width="2"
                        ...
                        @empty
                            <div class="col-span-full text-center py-12 text-indigo-100/80">
                                ...
                                <p class="text-sm text-indigo-100/70 mt-1">Catat unit baru untuk mulai memetakan aset per
                                    ruang.</p>
                            </div>
                        @endforelse
                    </div>

            ...
            <div class="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-12 space-y-6">
                <div class="flex items-center gap-3 flex-wrap">
                    ...
                    <div>
                        ...
                        <h1 class="text-3xl font-bold leading-tight mt-2">Edit ruang</h1>
                    </div>
                </div>

                <div class="bg-slate-900/80 border border-white/10 rounded-2xl shadow-xl shadow-indigo-500/15 overflow-hidden">
                    <form action="{{ route(($routePrefix ?? 'pegawai') . '.ruang.update', $ruang->idruang) }}" method="POST" class="p-6 space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <div>
                                ...
                                <select id="nama_gedung" 
                                        name="nama_gedung" 
                                        class="flex items-center w-full px-3 py-2 rounded-xl bg-slate-800/70 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent {{ $errors->has('nama_gedung') ? 'border-rose-400' : '' }}"
                                        required>
                                    ...
                                    <option value="Yayasan" {{ old('nama_gedung', $ruang->nama_gedung) == 'Yayasan' ? 'selected' : '' }}>Yayasan</option>
                                    <option value="Keamanan" {{ old('nama_gedung', $ruang->nama_gedung) == 'Keamanan' ? 'selected' : '' }}>Keamanan</option>
                                </select>
                                @error('nama_gedung')
                                    <p class="mt-1 text-sm text-rose-300">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                <label for="nama_lantai" class="block text-sm font-semibold text-indigo-100 mb-2">
                                    Lantai <span class="text-rose-300">*</span>
                                </label>
                            ...
                            <div>
                                ...
                                <textarea id="keterangan" name="keterangan" rows="3"
                                    ...
                                    placeholder="Masukkan keterangan (opsional)" maxlength="255">{{ old('keterangan', $ruang->keterangan) }}</textarea>
                                @error('keterangan')
                                    <p class="mt-1 text-sm text-rose-300">{{ $message }}</p>
                                @enderror
                            </div>

        ...
        <div class="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-12 space-y-6">
            <div class="flex items-center gap-3 flex-wrap">
                ...
                <div>
                    ...
                    <h1 class="text-3xl font-bold leading-tight mt-2">Edit kategori</h1>
                </div>
            </div>

            <div class="bg-slate-900/80 border border-white/10 rounded-2xl shadow-xl shadow-indigo-500/15 overflow-hidden">
                <form action="{{ route(($routePrefix ?? 'pegawai') . '.kategori.update', $kategori->idkategori) }}" method="POST" class="space-y-5 p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        ...
                        <div>
                            ...
                            <textarea id="keterangan" 
                                      ...
                                      maxlength="500">{{ old('keterangan', $kategori->keterangan) }}</textarea>
                            @error('keterangan')
                                <p class="mt-1 text-sm text-rose-300">{{ $message }}</p>
                            @enderror
...
document.getElementById('keterangan').addEventListener('input', function(e) {
    ...
    counter.className = remaining < 50 ? 'char-counter mt-1 text-sm text-rose-300' : 'char-counter mt-1 text-sm text-indigo-200/80';
});
</script>
@endsection
